'use strict';
const parseVirusesTotal = function (virusTotalInfo: any) {
    return {
        ...
        lastDnsRecords: virusTotalInfo.data.attributes.last_dns_records.map((record: any) => ({
            type: record.type,
            value: record.value,
            ttl: record.ttl,
        })),
        lastAnalysisResults: Object.entries(virusTotalInfo.data.attributes.last_analysis_results).map(([engineName, result]: [string, any]) => ({
            engineName,
            category: result.category,
            result: result.result,
            method: result.method,
        })),
        popularityRanks: Object.entries(virusTotalInfo.data.attributes.popularity_ranks).map(([rankSource, rankData]: [string, any]) => ({
            rankSource,
...
const parseWhoIsInfo = function (whoisInfo: any) {
    return {
